...
The binaries are currently unsigned, so Windows SmartScreen warns and reports the publisher as unknown. Every release publishes SHA-256 checksums, so you can check a download before running it.
...
The portable package is unsigned too, and the release page lists a checksum for it as well.
