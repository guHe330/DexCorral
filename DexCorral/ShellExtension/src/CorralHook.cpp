...
// Desktop sort modes — we own desktop sorting entirely (Fences-style).
...
// ============================================================================
// Compact visible icons — the hook's own layout/sort engine (Fences-style).
...
// ============================================================================
// Auto-sort popup - Fences-style "Disable auto-sort?" dialog
// ============================================================================
...
static LRESULT CALLBACK ListViewSubclassProcImpl(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    // hidden icons out of every layout operation (Fences-style immunity).
...
static LRESULT CALLBACK ShellDefViewSubclassProcImpl(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    if (uMsg == WM_COMMAND) {
        ...
        // Sort commands: the hook sorts the desktop itself (Fences-style).
    ...
    if (uMsg == WM_NOTIFY) {
        ...
        if (nmhdr->code == NM_CUSTOMDRAW && nmhdr->hwndFrom == g_hDesktopListView) {
            ...
            case CDDS_PREPAINT: {
                // Call original chain first so other hooks (e.g. Stardock Fences) get
            ...
            case CDDS_ITEMPREPAINT: {
                ...
                // Not a DexCorral icon - let the original chain (e.g. Fences) decide.
