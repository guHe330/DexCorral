...
- The Appearance dialog grew two rows (dialog height 308 → 350 dialog units): the Opacity group now holds labelled Background and Border sliders, and the Header group gained an Opacity row
...
- A corral could get stuck in resize (or drag) mode: after releasing the mouse button, moving the mouse kept resizing the corral. Corrals are background windows (`MA_NOACTIVATE`, pinned to `HWND_BOTTOM`, owned by Progman), and Win32 only delivers captured mouse messages to a background window while the cursor is over it — so a button released outside the corral never reached `OnLeftButtonUp` and `isResizing` stayed set forever. Two recoveries: `WM_MOUSEMOVE` now ends any captured operation when `wParam` reports the left button is no longer down (so a stuck corral heals on the very next mouse move), and a new `WM_CAPTURECHANGED` handler ends it when the capture is taken away — relevant because DexCorral's UI runs inside Explorer's process, where another component can grab the capture at any time. Both route through `EndCapturedOperationWithoutDrop`, which commits geometry (`EndResize` / `SyncConfigFromWindow`, so the released size and position are saved) but deliberately skips the position-dependent side effects of a real drop: no merge into another corral, no icon dropped on a guessed target, since the release point is unknown by then. Applies to window drag, corral resize, details-column resize, scrollbar drag, icon drag and tab reorder.
